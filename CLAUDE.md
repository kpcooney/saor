...
**State on disk**: `.claude/review-state/<branch>/<timestamp>/` (gitignored, per-developer, regenerable). Each run writes the three reviewer reports, the coordinator synthesis, and `meta.json` (branch, commit SHA, timestamp, run mode).
