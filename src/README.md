...
The frontend dev server runs automatically as part of `npm run tauri dev` (from the project root). Tauri's `beforeDevCommand` starts the Vite dev server on `http://localhost:1420`.
...
The frontend has no independent test suite in Phase 1 — testing effort is focused on the Rust backend and TypeScript agent layer where the core logic lives.
